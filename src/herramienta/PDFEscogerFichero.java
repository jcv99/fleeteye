...
public class PDFEscogerFichero extends JFileChooser {
	...
	public void guardarDocumento(PDDocument documento) {
		try {
			File documentoGuardado = new File(obtenerDestinacion().getAbsolutePath() + Messages.getString("BARRA") //$NON-NLS-1$
	...
	private File obtenerDestinacion()
	...
	public File obtenerOrigen() {
		...
		this.addChoosableFileFilter(new FileNameExtensionFilter(Messages.getString("PDFEscogerFichero.4"), //$NON-NLS-1$
				Messages.getString("PDFEscogerFichero.5"))); //$NON-NLS-1$
